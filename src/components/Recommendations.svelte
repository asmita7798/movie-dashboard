<script>
  ...
  onMount(async () => {
    ...
    popularPicks = [...imdbCSV]
      .sort((a, b) => parseInt(b.No_of_votes) - parseInt(a.No_of_votes))
      .slice(0, 10);

   
    topIMDBMovies = [...imdbCSV]
      .sort((a, b) => parseFloat(b.IMDB_Rating) - parseFloat(a.IMDB_Rating))
      .slice(0, 10);

    
    criticsPicks = [...imdbCSV]
  ...
  function updateRecommendations() {
    ...
    recommendedMovies = imdbCSV
      .filter(m => recommendedTitles.includes(m.Series_Title))
      .slice(0, 10);
  }
...
<div class="container">
  <div class="hero-header">
    ...
    <button class="home-btn" on:click={() => currentView = 'dashboard'}>
    ...
    <div class="scroll-row">
      {#each recommendedMovies as movie}
        <div class="card fade-in">
          <img src={movie.Poster_Link} loading="lazy" alt={movie.Series_Title} />
  ...
  <div class="scroll-row">
    {#each popularPicks as movie}
      <div class="card fade-in">
        <img src={movie.Poster_Link} alt={movie.Series_Title} loading="lazy" />
        <div class="title">{movie.Series_Title}</div>
      </div>
    {/each}
  </div>
